...
class UniappMethodChannel {
  ...
  void initChannel() {
    if (isInit) return;
    isInit = true;
    _channel = const MethodChannel('com.itfenbao.uniapp');
  ...
  void pop() {
    if (canPop()) {
      ...
      Future.delayed(const Duration(milliseconds: 10), () {
        fireEvent("canPop", Navigator.canPop(_context));
      });
    } else {
      fireEvent("pop");
    }
  ...
  Future $emitSync(String eventName, [dynamic arguments]) {
    ...
    var params = Map<String, dynamic>.from(arguments);
